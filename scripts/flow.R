...
diagnosis_order <- c("CTRL", "CIAP", "CIDP", "GBS", "MAG", "MFS", "PNC", "CAN", "PPN")
diagnosis_color <- setNames(pals::cols25(length(diagnosis_order)), diagnosis_order)
group_order <- c("CTRL", "PNP")
group_color <- setNames(pals::cols25(length(group_order)), group_order)

# load flow and lookup file ----
flow_pre <-
 read_excel(file.path("raw", "flow", "SEED_flow_v5.xlsx")) |>
...
# initalize stats
  ...
  for (par in x_var) {
    if (is.character(data[[par]])) {
      stats[[par]] <- rstatix::pairwise_fisher_test(table(data[[group]], data[[par]]), p.adjust.method = "BH") |>
        mutate(.y. = par) |>
    ...
    if (is.numeric(data[[par]])) {
      ...
      if (length(unique(data[[group]])) > 2) {
        stats[[par]] <- rstatix::dunn_test(as.formula(f_str), data = data, p.adjust.method = "BH") |>
          select(.y., group1, group2, p, p.adj, p.adj.signif)
      } else {
...
# PNP vs CTRL
stats_csf_con_group <- compStat(x_var = flow_vars, group = "group", data = flow$CSF)
...
csf_con_plots_patch_group <- patchwork::wrap_plots(csf_con_plots_group, ncol = 4)
ggsave(file.path("results", "flow", "csf_con_plots_group.pdf"), width = 5, height = 15, plot = csf_con_plots_patch_group)

# subgroups
stats_csf_con_dx <- compStat(x_var = flow_vars, group = "diagnosis", data = flow$CSF)
...
ggsave(file.path("results", "flow", "csf_con_plots_dx.pdf"), width = 10, height = 15, plot = csf_con_plots_patch_dx)


# blood flow boxplots -----

# PNP vs CTRL
stats_blood_con_group <- compStat(x_var = flow_vars, group = "group", data = flow$blood)
...
blood_con_plots_patch_group <- patchwork::wrap_plots(blood_con_plots_group, ncol = 4)
ggsave(file.path("results", "flow", "blood_con_plots_group.pdf"), width = 5, height = 15, plot = blood_con_plots_patch_group)

# subgroups
stats_blood_con_dx <- compStat(x_var = flow_vars, group = "diagnosis", data = flow$blood)
...
ggsave(file.path("results", "flow", "blood_con_plots_dx.pdf"), width = 10, height = 15, plot = blood_con_plots_patch_dx)
...
flow_pnp_ctrl_csf_pval <- statVolcano(flow_vars, reference = "group", data = flow$CSF)
flow_pnp_ctrl_csf_fc <- logfcVolcano(data = flow$CSF, group = "group", group1 = "PNP", group2 = "CTRL")
flow_pnp_ctrl_csf_vol_data <- left_join(flow_pnp_ctrl_csf_fc, flow_pnp_ctrl_csf_pval, join_by(var))
flow_pnp_ctrl_csf_vol_plot <- VolPlot(data = flow_pnp_ctrl_csf_vol_data, cols = flow_vars_cols, n = length(flow_vars))
ggsave(
...
flow_pnp_ctrl_blood_pval <- statVolcano(flow_vars, reference = "group", data = flow$blood)
flow_pnp_ctrl_blood_fc <- logfcVolcano(data = flow$blood, group = "group", group1 = "PNP", group2 = "CTRL")
flow_pnp_ctrl_blood_vol_data <- left_join(flow_pnp_ctrl_blood_fc, flow_pnp_ctrl_blood_pval, join_by(var))
flow_pnp_ctrl_blood_vol_plot <- VolPlot(data = flow_pnp_ctrl_blood_vol_data, cols = flow_vars_cols, n = length(flow_vars))
ggsave(
...
flow_ciap_ctrl_csf_pval <- statVolcano(flow_vars, reference = "diagnosis", data = flow_ciap_ctrl)
flow_ciap_ctrl_csf_fc <- logfcVolcano(data = flow_ciap_ctrl, group = "diagnosis", group1 = "CIAP", group2 = "CTRL")
flow_ciap_ctrl_csf_vol_data <- left_join(flow_ciap_ctrl_csf_fc, flow_ciap_ctrl_csf_pval, join_by(var))
flow_ciap_ctrl_csf_vol_plot <- VolPlot(data = flow_ciap_ctrl_csf_vol_data, cols = flow_vars_cols, n = length(flow_vars))
ggsave(
...
flow_cidp_ctrl_csf_pval <- statVolcano(flow_vars, reference = "diagnosis", data = flow_cidp_ctrl)
flow_cidp_ctrl_csf_fc <- logfcVolcano(data = flow_cidp_ctrl, group = "diagnosis", group1 = "CIDP", group2 = "CTRL")
flow_cidp_ctrl_csf_vol_data <- left_join(flow_cidp_ctrl_csf_fc, flow_cidp_ctrl_csf_pval, join_by(var))
flow_cidp_ctrl_csf_vol_plot <- VolPlot(data = flow_cidp_ctrl_csf_vol_data, cols = flow_vars_cols, n = length(flow_vars))
ggsave(
...
flow_gbs_ctrl_csf_pval <- statVolcano(flow_vars, reference = "diagnosis", data = flow_gbs_ctrl)
flow_gbs_ctrl_csf_fc <- logfcVolcano(data = flow_gbs_ctrl, group = "diagnosis", group1 = "GBS", group2 = "CTRL")
flow_gbs_ctrl_csf_vol_data <- left_join(flow_gbs_ctrl_csf_fc, flow_gbs_ctrl_csf_pval, join_by(var))
flow_gbs_ctrl_csf_vol_plot <- VolPlot(data = flow_gbs_ctrl_csf_vol_data, cols = flow_vars_cols, n = length(flow_vars))
ggsave(
...
flow_cidp_ciap_csf_pval <- statVolcano(flow_vars, reference = "diagnosis", data = flow_cidp_ciap)
flow_cidp_ciap_csf_fc <- logfcVolcano(data = flow_cidp_ciap, group = "diagnosis", group1 = "CIDP", group2 = "CIAP")
flow_cidp_ciap_csf_vol_data <- left_join(flow_cidp_ciap_csf_fc, flow_cidp_ciap_csf_pval, join_by(var))
flow_cidp_ciap_csf_vol_plot <- VolPlot(data = flow_cidp_ciap_csf_vol_data, cols = flow_vars_cols, n = length(flow_vars))
ggsave(
...
flow_gbs_ciap_csf_pval <- statVolcano(flow_vars, reference = "diagnosis", data = flow_gbs_ciap)
flow_gbs_ciap_csf_fc <- logfcVolcano(data = flow_gbs_ciap, group = "diagnosis", group1 = "GBS", group2 = "CIAP")
flow_gbs_ciap_csf_vol_data <- left_join(flow_gbs_ciap_csf_fc, flow_gbs_ciap_csf_pval, join_by(var))
flow_gbs_ciap_csf_vol_plot <- VolPlot(data = flow_gbs_ciap_csf_vol_data, cols = flow_vars_cols, n = length(flow_vars))
ggsave(
...
flow_ciap_ctrl_blood_pval <- statVolcano(flow_vars, reference = "diagnosis", data = flow_ciap_ctrl)
flow_ciap_ctrl_blood_fc <- logfcVolcano(data = flow_ciap_ctrl, group = "diagnosis", group1 = "CIAP", group2 = "CTRL")
flow_ciap_ctrl_blood_vol_data <- left_join(flow_ciap_ctrl_blood_fc, flow_ciap_ctrl_blood_pval, join_by(var))
flow_ciap_ctrl_blood_vol_plot <- VolPlot(data = flow_ciap_ctrl_blood_vol_data, cols = flow_vars_cols, n = length(flow_vars))
ggsave(
...
flow_cidp_ctrl_blood_pval <- statVolcano(flow_vars, reference = "diagnosis", data = flow_cidp_ctrl)
flow_cidp_ctrl_blood_fc <- logfcVolcano(data = flow_cidp_ctrl, group = "diagnosis", group1 = "CIDP", group2 = "CTRL")
flow_cidp_ctrl_blood_vol_data <- left_join(flow_cidp_ctrl_blood_fc, flow_cidp_ctrl_blood_pval, join_by(var))
flow_cidp_ctrl_blood_vol_plot <- VolPlot(data = flow_cidp_ctrl_blood_vol_data, cols = flow_vars_cols, n = length(flow_vars))
ggsave(
...
flow_gbs_ctrl_blood_pval <- statVolcano(flow_vars, reference = "diagnosis", data = flow_gbs_ctrl)
flow_gbs_ctrl_blood_fc <- logfcVolcano(data = flow_gbs_ctrl, group = "diagnosis", group1 = "GBS", group2 = "CTRL")
flow_gbs_ctrl_blood_vol_data <- left_join(flow_gbs_ctrl_blood_fc, flow_gbs_ctrl_blood_pval, join_by(var))
flow_gbs_ctrl_blood_vol_plot <- VolPlot(data = flow_gbs_ctrl_blood_vol_data, cols = flow_vars_cols, n = length(flow_vars))
ggsave(
