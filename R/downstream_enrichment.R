...
legacy_enrichment_workbooks <- function(root = "results/enrich") {
  paths <- list.files(
    root,
    pattern = "\\.xlsx$",
    recursive = TRUE,
  ...
  paths[grepl("/de_(combined|cluster)/(ora|gsea)/", paths)]
}

compare_enrichment_sheet <- function(current_path, legacy_path, sheet) {
  current <- readxl::read_xlsx(current_path, sheet = sheet)
  legacy <- readxl::read_xlsx(legacy_path, sheet = sheet)
  stopifnot(identical(names(current), names(legacy)), "ID" %in% names(current))

  current_ids <- as.character(current$ID)
  legacy_ids <- as.character(legacy$ID)
  if (!length(current_ids) && !length(legacy_ids)) {
    overlap <- 1
  } else {
    overlap <- length(intersect(current_ids, legacy_ids)) /
      length(union(current_ids, legacy_ids))
  }
  stopifnot(overlap >= 0.9)

  shared <- intersect(current_ids, legacy_ids)
  max_nes_difference <- 0
  if (length(shared) && "NES" %in% names(current)) {
    current_nes <- current$NES[match(shared, current_ids)]
    legacy_nes <- legacy$NES[match(shared, legacy_ids)]
    stopifnot(identical(sign(current_nes), sign(legacy_nes)))
    max_nes_difference <- max(abs(current_nes - legacy_nes), na.rm = TRUE)
    stopifnot(is.finite(max_nes_difference), max_nes_difference <= 0.1)
  }
  if (length(shared) && "Count" %in% names(current)) {
    stopifnot(identical(
      current$Count[match(shared, current_ids)],
      legacy$Count[match(shared, legacy_ids)]
    ))
  }
  ...
  tibble::tibble(
    ...
    legacy_rows = nrow(legacy),
    term_jaccard = overlap,
    max_abs_nes_difference = max_nes_difference,
    passed = TRUE
  )
}

validate_enrichment_workbooks <- function(
  generated_files,
  legacy_files,
  current_root = enrichment_result_dir(),
  legacy_root = "results/enrich"
) {
  generated <- generated_files[grepl("\\.xlsx$", generated_files)]
  stopifnot(length(generated) > 0L, all(file.exists(generated)))
  relative <- substring(legacy_files, nchar(legacy_root) + 2L)
  current <- file.path(current_root, relative)
  stopifnot(length(legacy_files) > 0L, all(file.exists(legacy_files)))

  available <- file.exists(current)
  stopifnot(all(available))
  purrr::map2_dfr(current, legacy_files, function(current_path, legacy_path) {
    current_sheets <- readxl::excel_sheets(current_path)
    legacy_sheets <- readxl::excel_sheets(legacy_path)
    stopifnot(identical(current_sheets, legacy_sheets))
    purrr::map_dfr(
      current_sheets,
      ~ compare_enrichment_sheet(current_path, legacy_path, .x)
    )
  })
}
