...
source(file.path("scripts", "correlation_helper.R"))
...
# Define severity scores to analyze
severity_scores <- c(
    "incat_at_lumbar_puncture",
    "incat_follow_up",
    "incat_progress",
...
# Run correlation analysis for all severity scores
correlation_results_csf <- run_severity_correlation_analysis(
    seurat_obj = sc_merge,
...
# Run correlation analysis for PBMC
correlation_results_blood <- run_severity_correlation_analysis(
    seurat_obj = sc_merge,
    severity_scores = severity_scores,
    lookup_data = lookup,
    tissue_type = "PBMC",
