...
# umap ---
umap_cluster <- 
  DimPlot(sc_merge, reduction = "umap.stacas.ss.all", pt.size = .1, raster = FALSE, alpha = 0.1, group.by = "RNA_snn_res.1", cols = my_cols_100, label = TRUE) +
  theme_rect() +
  NoLegend()

ggsave(plot = umap_cluster, file.path("results", "umap", "stacas_ss_all_res1_cluster.png"), width = 10, height = 10)

# split by cluster
umap_split_cluster <- 
  DimPlot(sc_merge, reduction = "umap.stacas.ss.all", pt.size = .1, raster = FALSE, alpha = 0.1, split.by = "RNA_snn_res.1", cols = my_cols_100, label = TRUE) +
  theme_rect() +
  NoLegend()

ggsave(plot = umap_split_cluster, file.path("results", "umap", "stacas_ss_all_split_res1.png"), width = 50, height = 5, limitsize = FALSE)
...
sc_cerebro@meta.data <-
    ...
    select(barcode, sample, tissue, diagnosis, nCount_RNA, nFeature_RNA, percent_mt, RNA_snn_res.1) |>
...
# annotate clusters ----
Idents(sc_merge) <- factor(sc_merge$cluster, levels = sort(unique(sc_merge$cluster)))
...
sc_merge@misc$cluster_col <- setNames(unname(Polychrome::createPalette(length(sc_merge@misc$cluster_order), colors_dutch)), sc_merge@misc$cluster_order)
sc_merge@misc$diagnosis_order <- c("CTRL", "CIAP", "GBS", "CIDP", "MAG", "MFS", "PNC", "CAN", "PPN")
sc_merge@misc$diagnosis_col <- setNames(pals::cols25(length(sc_merge@misc$diagnosis_order)), sc_merge@misc$diagnosis_order)
sc_merge@misc$tissue_diagnosis_order <- c(paste0("CSF_", sc_merge@misc$diagnosis_order), paste0("PBMC_", sc_merge@misc$diagnosis_order))
sc_merge@misc$tissue_diagnosis_col <- setNames(pals::cols25(length(sc_merge@misc$tissue_diagnosis_order)), sc_merge@misc$tissue_diagnosis_order)
...
umap <-
    DimPlot(sc_merge, reduction = "umap.stacas.ss.all", pt.size = .1, raster = FALSE, alpha = 0.1, group.by = "cluster", cols = sc_merge@misc$cluster_col, label = TRUE) +
    theme_rect() +
    NoLegend() + 
    xlab("UMAP1") + 
    ylab("UMAP2") + 
...
ggsave(plot = umap, file.path("results", "umap", "umap_annotated.png"), width = 10, height = 10)
